...
class SpeedLimitController:
  ...
  def final_cruise_speed(self) -> float:
    if self.is_active:
      ...
      if self.last_valid_speed_limit_offsetted > 0:
        return float(self.last_valid_speed_limit_offsetted)
  ...
  def transition_state_from_inactive(self) -> None:
    # if it's a new session, wait for 2 seconds after long engaged before transitioning to preActive
  ...
  def get_adapting_state_target_acceleration(self) -> float:
    if self._distance > 0:
      return float((self.speed_limit_offseted ** 2 - self.v_ego ** 2) / (2. * self._distance))

    return float(self.v_offset / float(ModelConstants.T_IDXS[CONTROL_N]))
